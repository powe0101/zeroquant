...
pub async fn sync_indicators(
    ...
    let target_symbols = if let Some(ref tickers) = symbols {
        ...
        get_stale_indicator_symbols(pool, stale_threshold, config.fundamental_collect.batch_size)
            .await?
    };
    ...
    for (symbol_info_id, ticker, market, yahoo_symbol) in target_symbols {
...
/// OHLCV 캔들 데이터 조회.
/// ohlcv 테이블의 symbol 컬럼은 yahoo_symbol 또는 ticker를 저장합니다.
/// yahoo_symbol이 있으면 우선 시도하고, 데이터가 없으면 ticker로 재시도합니다.
async fn get_candles(
    ...
    // 조회할 심볼 목록 생성 (yahoo_symbol 우선, 없으면 ticker)
    let symbols_to_try: Vec<&str> = match yahoo_symbol {
        Some(ys) if ys != ticker => vec![ys, ticker],
        _ => vec![ticker],
    };
    ...
    for symbol in &symbols_to_try {
        ...
        if !rows.is_empty() {
            ...
            let mut candles: Vec<Kline> = rows
                .into_iter()
                .map(|(open_time, open, high, low, close, volume, close_time)| Kline {
