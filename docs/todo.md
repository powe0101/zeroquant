...
| 백엔드: 백테스트 API에서 등록된 전략 ID로 실행 지원 | ✅ 완료 |
| 프론트: Backtest.tsx에서 등록된 전략만 표시 | ✅ 완료 |
| 프론트: 파라미터 입력 SDUI 폼 제거 (등록된 설정 사용) | ✅ 완료 |
| 프론트: Simulation.tsx 동일 변경 | ✅ 완료 |
| 전략 페이지에서 모든 전략 등록 테스트 | ✅ 완료 (27개 전략) |
| 백테스트 페이지에서 등록된 전략 테스트 | ✅ 완료 |
| 시뮬레이션 페이지에서 등록된 전략 테스트 | ✅ 완료 |
| UI 버그: "사용자정의" 탭 필터링 수정 | ✅ 완료 |

## 🔨 진행 중인 작업 (2026-01-31)

### 🔧 코드베이스 개선 로드맵 실행 (2026-01-31)

> **참조**: `docs/improvement_roadmap.md` - Phase 1~7 개선 계획

#### 완료된 작업 ✅

**1. 통합 에러 응답 타입 생성** (`crates/trader-api/src/error.rs`)
- `ApiErrorResponse` 통합 타입 정의 (기존 4개 타입 통합)
- Type alias로 점진적 마이그레이션 지원: `ApiError`, `BacktestApiError`, `SimulationApiError`
- `ApiResult<T>` 결과 타입 별칭 추가
- 단위 테스트 5개 추가

**2. 위험도 상 unwrap() 수정**
- `analytics.rs:920-924, 964-968`: `first()/last().unwrap()` → `match` 패턴으로 안전하게 변경
- `simulation.rs:553`: `engine.started_at.unwrap()` → `ok_or_else()?`로 에러 전파

**3. 테스트 코드 호환성 수정**
- `simulation.rs` 테스트에서 `engine.start()` 호출 시 누락된 인자 추가

#### 진행 상황
| Phase | 작업 | 상태 |
|-------|------|------|
| Phase 1 | Critical 개선사항 (에러 핸들링) | ✅ 완료 |
| Phase 7.1 | 코드 중복 제거 - 에러 응답 타입 통합 | ✅ 완료 |
| Phase 7.2 | 대형 파일 분리 (analytics.rs, Dataset.tsx) | ⏳ 대기 |
| Phase 6 | Rust API 최신 패턴 적용 | ⏳ 대기 |

#### 분석 결과 요약 (서브에이전트 분석)
- **unwrap() 159개**: 프로덕션 39개, 테스트 120개 → 위험도 상 5개 수정 완료
- **에러 타입 4개 중복**: `ApiErrorResponse`로 통합 완료
- **대형 파일**: analytics.rs(2,679줄), Dataset.tsx(2,198줄) → 분리 작업 대기

---

### ✅ 다중 자산 전략 심볼 비교 버그 수정 (2026-01-31)

**문제**: 다중 자산 전략(All Weather, Snow, BAA 등)에서 백테스트 시 거래가 발생하지 않음 (0% 수익률)

**원인 분석**:
- `data.symbol.to_string()` 반환값: "SPY/USD" 형식 (base/quote)
- `asset_data` 키 저장 형식: "SPY" (base만)
- 심볼 비교 불일치로 모든 시장 데이터가 무시됨

**해결 방법**: `data.symbol.to_string()` → `data.symbol.base.clone()` 변경

| 파일 | 함수 | 상태 |
|------|------|------|
| all_weather.rs | on_market_data | ✅ 이전 세션에서 수정됨 |
| snow.rs | on_market_data | ✅ 수정됨 |
| baa.rs | on_market_data | ✅ 수정됨 |
| dual_momentum.rs | on_market_data, on_order_filled | ✅ 수정됨 |
| sector_momentum.rs | on_market_data, on_order_filled | ✅ 수정됨 |
| market_cap_top.rs | on_market_data | ✅ 수정됨 |
| us_3x_leverage.rs | on_market_data | ✅ 수정됨 |
| kospi_bothside.rs | on_market_data | ✅ 수정됨 |
| kosdaq_fire_rain.rs | on_market_data | ✅ 수정됨 |
| sector_vb.rs | on_market_data | ✅ 수정됨 |

**참고 - 단일 자산 전략 (수정 불필요)**:
- rsi.rs, bollinger.rs, grid.rs, sma.rs 등
- `config.symbol`이 "BTC/USDT" 형식으로 저장되어 `data.symbol.to_string()`과 일치

**검증 완료**:
- [x] 통합 테스트 실행 (cargo test -p trader-analytics) - 29개 테스트 통과
- [x] All Weather 백테스트 API 검증 - **+9.4% 수익률**, 각 심볼별 다른 가격 확인
- [x] 다른 다중 자산 전략 백테스트 검증 (2026-01-31)

**다중 자산 전략 API 백테스트 검증 결과** (2026-01-31):

| 전략 | 수익률 | MDD | 거래수 | 가격 매칭 | 상태 |
|------|--------|-----|--------|----------|------|
| All Weather | +9.40% | 1.37% | 6 | ✅ SPY $471→$637, TLT $95→$77 | ✅ 정상 |
| HAA | +1.57% | 0.84% | 24 | ✅ SPY $502, IEF $102, VWO $40 | ✅ 정상 |
| XAA | +2.45% | 1.30% | 6 | ✅ IEF $79~$96 | ✅ 정상 |
| Simple Power | +4.35% | 13.0% | 23 | ⚠️ 샘플 데이터 사용 | ✅ 작동 |
| BAA | - | - | 0 | - | ⚠️ 데이터 없음 |
| Snow | - | - | 0 | - | ⚠️ 데이터 없음 |
| Dual Momentum | - | - | 0 | - | ⚠️ 데이터 없음 |

**결론**: 가격 매칭 버그 수정 완료. 데이터가 있는 전략은 모두 심볼별 다른 가격으로 정상 거래됨.

---

### ✅ 다중 자산 백테스트 가격 매칭 버그 수정 (2026-01-31)

**문제**: 다중 자산 전략 백테스트 시 모든 심볼이 동일한 가격으로 거래됨

**증상** (스크린샷에서 발견):
- 올웨더 전략: 모든 6개 심볼(SPY, TLT, IEF, GLD, PDBC, IYK)이 동일한 진입가 (₩284)와 청산가 (₩96)로 기록
- 실제 ETF 가격(SPY $450, TLT $95 등)과 전혀 맞지 않음
- 결과적으로 -65.66% 손실 (비정상 결과)

**원인 분석**:
```rust
// BacktestEngine.open_position / close_position에서
let base_price = signal.suggested_price.unwrap_or(kline.close);  // 문제!
```
- 다중 자산 전략에서 여러 신호가 **동시에** 반환됨
- 모든 신호가 **현재 처리 중인 kline(다른 심볼)**의 가격으로 체결됨
- 예: SPY kline 처리 중 → TLT, IEF 신호도 SPY 가격으로 체결!
...
let base_price = signal
    .suggested_price
    .or_else(|| self.current_prices.get(&key).copied())  // 해당 심볼의 가격!
    .unwrap_or(kline.close);  // fallback
```

| 파일 | 함수 | 수정 내용 | 상태 |
|------|------|---------|------|
| trader-analytics/src/backtest/engine.rs | open_position | current_prices에서 해당 심볼 가격 사용 | ✅ |
| trader-analytics/src/backtest/engine.rs | close_position | current_prices에서 해당 심볼 가격 사용 | ✅ |

**검증 테스트 추가**:
| 테스트 | 검증 내용 | 파일 |
|--------|---------|------|
| test_multi_asset_price_matching | 각 거래의 가격이 해당 시점의 해당 심볼 kline.close와 일치하는지 검증 | strategy_integration.rs |

**테스트 결과**:
```
✅ SPY - 진입: $427.89 ≈ $427.89 (데이터셋 가격)
✅ TLT - 진입: $90.33  ≈ $90.33  (데이터셋 가격)
✅ IEF - 진입: $95.08  ≈ $95.08  (데이터셋 가격)
✅ GLD - 진입: $171.15 ≈ $171.15 (데이터셋 가격)
✅ PDBC - 진입: $14.26 ≈ $14.26 (데이터셋 가격)
✅ IYK - 진입: $57.05  ≈ $57.05  (데이터셋 가격)
✅ 다중 자산 가격 매칭 검증 통과!
```
...
### UI 회귀 테스트 ✅ 완료 (2026-01-31)
| 작업 | 상태 |
|------|------|
| 27개 전략 UI 등록 | ✅ Playwright 자동화 테스트 완료 |
| 백테스트 워크플로우 테스트 | ✅ 올웨더, 볼린저 밴드 검증 완료 |
| 시뮬레이션 워크플로우 테스트 | ✅ 볼린저 밴드 검증 완료 |
| 전략 선택 모달 카테고리 필터링 | ✅ "사용자정의" 탭 분리 버그 수정 |
